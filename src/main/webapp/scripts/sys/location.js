...
function inntlisttree(){
	...
	function addHoverDom(treeId, treeNode) {
		...
		if (btnadd) btnadd.bind("click", function(){
			$("#itemForm input[name=ID]").removeAttr("readonly");
		...
		if (btnedit) btnedit.bind("click", function(){
			$("#itemForm input[name=ID]").attr("readonly","readonly");
	...
	$.ajax({
		...
		success:function(data){
			data.push({id:0,name:"地区信息字典",open:true});
...
function btn_save_click(){
	...
	if ($("#itemForm input[name=ID]").attr("readonly")) {
		url = "location/edit";
	}
	var form = $("#itemForm");
	if (form.valid()) {
